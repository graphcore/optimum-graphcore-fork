...
from ..t5.modeling_t5 import CustomT5Stack, CustomT5Block
...
class PipelinedMT5ForConditionalGeneration(MT5ForConditionalGeneration, PipelineMixin, IPUGenerationMixin):
    ...
    def parallelize(self, for_generation=False):
        ...
        # split large embedding across the first two IPUs, placing x% of serialized embeddings on the first IPU
        # split_ipu = int(len(self.shared.shared.split_embeddings) * 0.9)
        # self.shared.shared.split_embeddings[0] = poptorch.BeginBlock(self.shared.shared.split_embeddings[0], "Embedding_0", ipu_id=0)
        # self.shared.shared.split_embeddings[split_ipu] = poptorch.BeginBlock(self.shared.shared.split_embeddings[split_ipu], "Embedding_1", ipu_id=1)
        ...
        if for_generation:
            logger.info(f"LM Head Output --> IPU {ipu + 1}")
            if (ipu + encoder_layer_ipu[-1] + 2) != self.ipu_config.ipus_per_replica:
                ipu += 1
            self.lm_head.split_linear_layers[0] = poptorch.BeginBlock(self.lm_head.split_linear_layers[0], f"LM Head Output {0}", ipu_id=ipu)
        else:
            # TODO: need to make sure that splitting the lm head does not exceed ipus per replica for training
            logger.info(f"LM Head Output --> IPU {ipu}-{ipu + 1}")
            self.lm_head.split_linear_layers[0] = poptorch.BeginBlock(self.lm_head.split_linear_layers[0], f"LM Head Output {0}", ipu_id=ipu)
            for i, l in enumerate(self.lm_head.split_linear_layers[1:]):
                self.lm_head.split_linear_layers[i+1] = poptorch.BeginBlock(self.lm_head.split_linear_layers[i+1], f"LM Head Output {i+1}", ipu_id=ipu + 1)
