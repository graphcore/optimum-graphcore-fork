...
def _int4_npy_decoder_v1(data: NDArray) -> NDArray:
    """Int4 NumPy decoder.
...
def _int4_npy_encoder_v1(data: NDArray) -> NDArray:
    """Int4 encoder.
...
def group_quantize_compress(t: torch.Tensor, group_size: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    t_packed = _int4_npy_encoder_v1(t_quantized.flatten())
...
def group_quantize_decompress(
    ...
    if poptorch.isRunningOnIpu():
        ...
        t_unpacked = torch.tensor(_int4_npy_decoder_v1(t_packed).reshape(n_rows, n_groups, n_group_ids * 4))
        # t_unpacked = (
...
def main():
    ...
    t_scale = torch.ones(768, n_groups, 1)
    t_bias = torch.zeros(768, n_groups, 1)

    class Model(nn.Module):
        def forward(self, t_packed, t_scale, t_bias):
            return group_quantize_decompress(t_packed, t_scale, t_bias)
